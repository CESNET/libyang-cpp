 ...
 * Wraps `lysc_path`.
 */
std::string SchemaNode::path(SchemaPathType schemaPathType) const
{
    // TODO: support all path formats
    auto strDeleter = std::unique_ptr<char, deleter_free_t>(lysc_path(m_node, utils::toSchemaPathType(schemaPathType), nullptr, 0));
    if (!strDeleter) {
        throw std::bad_alloc();
    }
