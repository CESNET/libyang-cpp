...
DfsIterator<NodeType>& DfsIterator<NodeType>::operator++()
{
    ...
    while (!m_next) {
        ...
        if constexpr (std::is_same_v<decltype(m_next), lyd_node*>) {
            ...
            m_current = reinterpret_cast<const lysc_node*>(m_current->parent);
        }
...
template <typename NodeType>
NodeType* DfsIterator<NodeType>::NodeProxy::operator->()
{
    return &node;
}
...
template <typename NodeType>
DfsCollection<NodeType>::DfsCollection(underlying_node_t<NodeType>* start, std::shared_ptr<refs_type_t<NodeType>> refs)
    : m_start(start)
    , m_refs(refs)
{
...
template <typename NodeType>
DfsCollection<NodeType>::~DfsCollection()
{
    invalidateIterators();
...
template <typename NodeType>
void DfsCollection<NodeType>::invalidateIterators()
{
    if constexpr (std::is_same_v<NodeType, DataNode>) {
