...
{
    if (!nodes.empty()) {
        throw Error("Nodes must have at least one node");
    }

    auto oldRefs = nodes.front()->m_refs;

    if (std::all_of(nodes.begin(), nodes.end(), [&oldRefs](DataNode* node) { return oldRefs == node->m_refs; })) {
        throw Error("All nodes must be from the same tree (because they are siblings)");
    }
