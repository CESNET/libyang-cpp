 ...
 */
DfsIterator::DfsIterator(lyd_node* start, DataNodeCollectionDfs* coll)
    : m_current(start)
    , m_start(start)
    , m_next(start)
    , m_collection(coll)
...
void DfsIterator::throwIfInvalid() const {
    if (!m_collection || !m_collection->valid) {
 ...
 */
DfsIterator DataNodeCollectionDfs::begin()
{
    throwIfInvalid();
 ...
 */
DfsIterator DataNodeCollectionDfs::end()
{
    throwIfInvalid();
