...
namespace {
void impl_freeModuleData(void* moduleData, void* userData)
{
    auto cb = reinterpret_cast<libyang::Context::ModuleCallbacks*>(userData);
    if (cb->deleter) {
        cb->deleter(moduleData);
    }
}
...
{
    auto cb = reinterpret_cast<libyang::Context::ModuleCallbacks*>(userData);
    auto ret = cb->callback(modName, modRev, submodName, submodRev);
    if (!ret) {
        return LY_ENOT;
    }
 ...
 */
void Context::registerModuleCallback(std::function<ModuleCallback> callback, std::function<void(void*)> deleter)
{
    if (!callback) {
        throw std::logic_error("Context::registerModuleCallback: callback is empty.");
    }

    m_callbacks.callback = std::move(callback);
    m_callbacks.deleter = std::move(deleter);
    ly_ctx_set_module_imp_clb(m_ctx.get(), impl_callback, &m_callbacks);
}
}
