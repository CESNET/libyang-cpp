    ...
    : m_ctx(nullptr, nullptr) // fun-ptr deleter deletes the default constructor
{
    ...
    if (err != LY_SUCCESS) {
        throw LibyangErrorCode("Can't create libyang context (" + std::to_string(err) + ")", err);
    }
...
void Context::parseModuleMem(const char* data, const SchemaFormat format)
{
    ...
    if (err != LY_SUCCESS) {
        throw LibyangErrorCode("Can't parse module (" + std::to_string(err) + ")", err);
    }
}
...
DataNode Context::parseDataMem(const char* data, const DataFormat format)
{
    ...
    if (err != LY_SUCCESS) {
        throw LibyangErrorCode("Can't parse data (" + std::to_string(err) + ")", err);
    }
