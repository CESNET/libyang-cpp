...
Context createUnmanagedContext(ly_ctx* ctx, std::function<void(ly_ctx*)> = nullptr);
...
public:
    ...
    friend Context createUnmanagedContext(ly_ctx* ctx, std::function<void(ly_ctx*)>);
    friend ly_ctx* retrieveContext(Context ctx);

private:
    Context(ly_ctx* ctx, std::function<void(ly_ctx*)> = nullptr);
