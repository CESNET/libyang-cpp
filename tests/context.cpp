  ...
  <person xmlns="http://example.com/coze">
    ...
    DOCTEST_SUBCASE("Context::parseValueFragment")
    {
        DOCTEST_SUBCASE("String") {
            ...
            REQUIRE(node);
            REQUIRE(*node->printStr(libyang::DataFormat::JSON, libyang::PrintFlags::Shrink) == "{\"example-schema:dummy\":\"hello\"}");
        }
        ...
        DOCTEST_SUBCASE("List key") {
            ...
            REQUIRE(node);
            REQUIRE(*node->printStr(libyang::DataFormat::JSON, libyang::PrintFlags::Shrink) == "{\"example-schema:person\":[{\"name\":\"hello\"}]}");
        }
    }
