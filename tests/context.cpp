...
TEST_CASE("context")
{
    ...
    DOCTEST_SUBCASE("Context::parseExt")
    {
        ...
        DOCTEST_SUBCASE("ietf-restconf")
        {
            ...
            auto node = ctx->parseExt(ext, R"({"ietf-restconf:errors": {"error": [{"error-type": "protocol", "error-tag": "invalid-attribute", "error-message": "hi"}]}})", libyang::DataFormat::JSON);
            REQUIRE(node);
    ...
    <value>
      <song xmlns="http://example.com/ns/example-jukebox">
        ...
        <length>259</length>
            ...
            auto node = ctx->parseExt(ext, data, libyang::DataFormat::XML);
            REQUIRE(node);
